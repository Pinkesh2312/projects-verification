...
void server2(vector<int> &client_sockets, string msg, vector<string> required_files, vector<string> my_files, int unique_id, string my_path, int &master_socket)
{
	...
	// sending files if I have to respective clients
	// vector<int> clsocket;
	// vector<string> asked;
	// for (auto i : client_sockets)
	// {	
	...
	// vector<string> messages_from_two_hop_away;
	// vector<int> socket;

	// for (auto i : client_sockets2)
	// {
	// 	memset(buffer, 0, 10000);
	// 	readval = recv(i, &buffer, 10000, 0);
	// 	vector<string> received_files = get_file_list(string(buffer), " ");
	// 	socket.push_back(i);
	// 	messages_from_two_hop_away.push_back(string(buffer));
	// }

	// for(auto i:socket){
	// 	vector<string> files_to_be_sent = get_file_list(messages_from_two_hop_away[i], " ");
	// 	if(files_to_be_sent.size()>0){
	// 		for (auto j : files_to_be_sent)
	// 		{
	// 			string path = my_path + "/" + j;
	// 			sentval=send_server_image(socket[i], path);
	// 		}
	// 	}
	// }
}
...
void client(vector<int> neighbouring_ports, vector<int> &sockfd, vector<string> required_files, vector<string> my_files, string my_path, int unique_id)
{
	...
	// for(auto a:two_hop_away_ports){
	// 	cout << a.first << " " << a.second << endl;
	// }

	// // sending messages to figure out count 
	for(int j = 0; j < number_of_neighbours; j++){
		msg=to_string(unique_id) + " ";
	...
	// for (int i = 0; i < ports_to_be_contacted.size(); i++)
	// {
	// 	msg = "";
	// 	for(auto file:two_hop_away_ports){
	// 		if(file.second == ports_to_be_contacted[i]){
	// 			msg = msg + file.first + " ";
	// 		}
	// 	}
	// 	n = msg.length();
	// 	sentval = send(port_to_sockfd[ports_to_be_contacted[i]], msg.c_str(), n + 1, 0);
	// }
	...
	// 	for (auto file : two_hop_away_ports)
	// 	{
	// 		if (file.second == sockfd_to_port[socket_fds[i]])
	// 		{
	// 			string p = my_path + "/Downloaded/";
	// 			int status = mkdir(p.c_str(),0777);
	// 			string path = p + file.first;
	// 			readval = recv_client_image(socket_fds[i], path);
	// 		}
	// 	}
	// }
